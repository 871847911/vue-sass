  ...
  <div>
    <div class="show-img mb-30">
      <div v-for="(item,index) in imgLists" class="imgList mr-20 pos-r">
        ...
        <img :src="resDomain?(resDomain+item):(imgHostTemp+item)" alt="" />
      </div>
    ...
    <div class="upload pos-r mb-30" v-if="imgLists.length+imgListsTemp.length<maxNum">
...
<script type="text/javascript">
  ...
  import { imgHost, imgHostTemp } from "@/config/config";
  export default {
    props: {
      imgLists: {
        type: Array,
    ...
    methods: {
      uploadFileFun(e) {
        ...
        uploadFile(uptoken, file, filePath, (res, state) => {
          ...
          if (state.success) {
            self.imgListsTemp.push(res.key);
          } else {
            this.$message({
              message: '文件上传失败',
              type: "error"
      ...
      deleteImg(index, type='') {
        let self = this
        self[`imgLists${type}`].splice(index, 1)
      }
    }
  };
</script>
