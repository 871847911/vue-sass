<template>
  <div class="app-container">
    <tit></tit>
    <div class="tabBox">
        <div class="tabBoxHeader">推客：<span>{{storeName}}</span></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getCatalogPage(page = 1, pageSize) {
      ...
      self.loading = true;
      api.settlementRecordsh(data).then(res => {
        console.log(res)
        self.loading = false;
    ...
    handleClick(val) {
      console.log(val)
      this.$router.push({ name: "pushRecordDetails",query: {id:this.$route.query.id, date: val.date , name:this.$route.query.name ,status:val.status}});
    },
    //条件查询
    handleQuery(){
      this.getCatalogPage()
